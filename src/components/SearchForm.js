...
/** SearchForm contains the list of carMakes name an upload component. */
export default class SearchForm extends Component {
  ...
  },
  {
   ...
   this.renderCarItems = this.renderCarItems.bind(this);
   this.renderCarModels = this.renderCarModels.bind(this);
   this._handleSubmit = this._handleSubmit.bind(this);
   this._handleOnChange = this._handleOnChange.bind(this);
}


/** map through the object into categories by looking up the phrases. */
renderCarItems() {
  let carMenu = this.state.Makes.map((item) => {
  ...
  renderCarModels() {
    let carModels = this.state.Model.map((item) => {
        return <option value={item.name}>{item.name}</option>
      });
      return carModels;
    }
    ...
    _handleSubmit(e) {
      ...
      /** Details routes for the selectes car model */
      /** after this set up the details router */
      this.context.router.history.push('/details/$(selectValue)');
    }
    /** upadate the target by select the value. */
    _handleOnChange(e) {
      this.setstate({
  ...
  render(){
    return(
      <form onSubmit={this._handleSubmit}>
        <select type="select" value={this.state.selectValue} onChange={this._handleOnChange.bind(this)} className="select-box">
          {this.renderCarItems()}
        </select>
